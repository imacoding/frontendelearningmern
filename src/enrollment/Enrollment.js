...
export default function Enrollment() {
  ...
  useEffect(() => {
    ...
    read({ enrollmentId }, { t: jwt.token }, signal).then((data) => {
      if (data.error) {
  ...
  const totalCompleted = (lessons) => {
    let count = lessons.reduce((total, lessonStatus) => total + (lessonStatus.complete ? 1 : 0), 0);
  ...
  const markComplete = () => {
    if (!enrollment.lessonStatus[values.drawer].complete) {
      ...
      let updatedData = {
        lessonStatusId: lessonStatus[values.drawer]._id,
        complete: true,
      };
      ...
      complete({ enrollmentId }, { t: jwt.token }, updatedData).then((data) => {
        if (data && data.error) {
          setValues({ ...values, error: data.error });
        } else {
  ...
  return (
    <div className={classes.root}>
      ...
      >
        ...
        <List>
          <ListItem button onClick={() => navigate(-1)} className={values.drawer === -1 ? classes.selectedDrawer : classes.unselected}>
            <ListItemIcon>
              <Info />
            </ListItemIcon>
        ...
        <>
          ...
          <Card className={classes.card}>
            <CardHeader
              ...
              action={
                <Button onClick={markComplete} variant={enrollment.lessonStatus[values.drawer].complete ? 'contained' : 'outlined'} color="secondary">
